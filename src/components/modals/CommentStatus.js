...
const CommentStatus = () =>{
  ...
  const modal = useSelector( state => state.modals.commentStatus);


  function closeModal(cancel = true){
    console.log('close modal')
    dispatch({type:"CLOSE_MODAL", modal:"commentStatus"})
  ...
  function saveStatus(){
    ...
    dispatch(asyncChangeStatus(modal.id, modal.status,comment,0, closeModal));
  }
  ...
  return <ModalLayout>
          ...
          <div className="modal-content">
            <div className="modal-header">
                <h5 className="modal-title" id="exampleModalLabel">Введите комментарий</h5>
