...
const DeskColumn = ({droppableId,items,status, statusName, bottom}) => {
  ...
  return <Droppable droppableId={droppableId} >
    {(provided, snapshot) => (
      <div className="card droppable" id={droppableId}
        ref={provided.innerRef}
        style={getListStyle(snapshot.isDraggingOver)}
      >
          <div className="card-header mb-3" style={{minHeight:'56px'}}>{statusName}</div>
        {bottom ? '' : items.map((item, index) => (
          <Draggable key={item.id} draggableId={item.id.toString()} index={item.id} phone={item.phone}>
            {(provided, snapshot) => (
              <div className="card"
                ref={provided.innerRef}
