...
export const asyncChangeStatus =
  (id, status, comment = "", time = "", closeFunc = 0) =>
  (dispatch, getState) => {
    //dispatch({type:"LOADING"});
    ...
    API.put(`candidates/${id}/change-status`, {
      status: status,
      comment: comment,
      time: time,
    })
...
export const asyncGetDeskCandidates =
  (filter = false) =>
  (dispatch, getState) => {
    dispatch({ type: "LOADING" });
...
export const asyncGetCardCandidate =
  (id, setLoading) => (dispatch, getState) => {
    dispatch({ type: "LOADING" });
    const state = getState();
    const API = api(dispatch, state);
...
export const asyncGetCandidatesFilter = () => (dispatch, getState) => {
  const state = getState();
  const API = api(dispatch, state);
  API.post(`candidates/filter`, { filter: state.filterTable }).then(function (
    response
  ) {
...
export const asyncGetInitSettings = () => (dispatch, getState) => {
  const state = getState();
  const API = api(dispatch, state);
